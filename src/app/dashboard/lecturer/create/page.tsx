...
import { MousePointer2, FileSpreadsheet, ArrowLeft, Save, Plus, Trash2, Info, Clock } from 'lucide-react';
...
export default function CreateExamPage() {
  ...
  const [duration, setDuration] = useState(45); // Default timer set to 45 mins
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      // 1. Insert Exam Header (Now with dynamic duration)
      const { data: exam, error: examError } = await supabase
        ...
        .insert([{
          ...
          level: level,
          total_marks: 25,
          duration_minutes: duration, // Uses the lecturer's set time
          is_active: true
        }])
        .select()
        .single();
        ...
        <div className="max-w-4xl mx-auto py-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <button 
              ...
              className="group p-8 bg-white border-2 border-slate-200 rounded-2xl hover:border-green-500 hover:shadow-xl transition-all text-left"
            >
            ...
            <div className="bg-blue-50 p-4 rounded-lg flex gap-3 border border-blue-100 mb-8">
              ...
              <div>
                ...
                <p className="text-xs text-blue-700">
                  Headers must be: <strong>Question, Option A, Option B, Option C, Option D, Correct Index</strong> (0=A, 1=B, 2=C, 3=D).
                </p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="md:col-span-2">
                  <Input 
                    label="Exam Title" 
                    placeholder="Enter title before uploading" 
                    value={examTitle}
  ...
  return (
    <DashboardLayout role="Manual Entry">
      <div className="max-w-4xl mx-auto pb-20">
        ...
        {/* --- Dynamic Header for Title and Timer --- */}
        ...
        <div className="space-y-6">
          {questions.map((q, qIdx) => (
            <div key={qIdx} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm relative animate-in fade-in slide-in-from-bottom-2">
              <div className="flex justify-between items-start mb-4">
                <span className="bg-slate-100 text-slate-600 px-3 py-1 rounded-md text-sm font-bold">Question {qIdx + 1}</span>
                <button onClick={() => removeQuestion(qIdx)} className="text-slate-400 hover:text-red-500 transition-colors">
                  <Trash2 size={20} />
                </button>
              </div>
              
              <textarea 
                className="w-full p-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-slate-900 mb-4 transition-all"
                rows={2}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {q.options.map((opt, oIdx) => (
                  <div key={oIdx} className="flex items-center gap-2">
                    ...
                    />
                    <input 
                      className="flex-1 p-2 border border-slate-200 rounded-md text-sm text-slate-900"
                      value={opt}
                      onChange={(e) => {
                        const newQs = [...questions];
        ...
        <button 
          onClick={addQuestion}
          className="w-full mt-6 py-4 border-2 border-dashed border-slate-300 rounded-xl text-slate-500 hover:border-blue-400 hover:text-blue-500 font-medium flex items-center justify-center gap-2 transition-all"
        >
